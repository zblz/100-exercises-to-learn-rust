...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::DescriptionError { .. }) => Ticket::new(
            title.clone(),
            "Description not provided".into(),
            status.clone(),
        )
        .unwrap(),
        Err(TicketNewError::TitleError { description }) => panic!("{}", description),
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::TitleError {
                description: "Title cannot be longer than 50 bytes".to_string(),
            });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError {
                description: "Description cannot be empty".to_string(),
            });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError {
                description: "Description cannot be longer than 500 bytes".to_string(),
            });
        }
